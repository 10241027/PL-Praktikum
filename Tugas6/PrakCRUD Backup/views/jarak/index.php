...
<div class="">
    ...
    <div class="toolbar">
        <form method="GET">
            <input type="hidden" name="c" value="jarak">
            <input type="hidden" name="a" value="index">

            <input type="text" name="q" placeholder="Cari jarak / deskripsi..." 
            ...
            <div class="sort-group">
                <select name="sort">
                    ...
                    <option value="deskripsi" <?= (isset($_GET['sort']) && $_GET['sort']=='deskripsi') ? 'selected' : '' ?>>Deskripsi</option>
                </select>
                ...
                <div class="sort-buttons">
                    ...
                    ?>
                    <a href="?c=jarak&a=index&sort=<?= $currentSort ?>&order=asc&q=<?= $q ?>" 
                       class="button <?= $currentOrder==='asc'?'active':'' ?>">🔼</a>
                    <a href="?c=jarak&a=index&sort=<?= $currentSort ?>&order=desc&q=<?= $q ?>" 
                       class="button <?= $currentOrder==='desc'?'active':'' ?>">🔽</a>
                </div>
            </div>

            <button type="submit">🔍 Cari</button>
            <a href="?c=jarak&a=index" class="button">🔄 Reset</a>
        </form>
    </div>
    ...
    if (isset($_GET['q']) && $_GET['q'] !== '') {
        ...
        $data = array_filter($data, function($item) use ($q) {
            return strpos(strtolower($item['jarak_km']), $q) !== false ||
                   strpos(strtolower($item['deskripsi']), $q) !== false;
        });
    }
    ...
    <div class="pagination">
        <?php if ($page > 1): ?>
            <a class="button" href="?c=jarak&a=index&page=<?= $page - 1 ?>">⟵ Sebelumnya</a>
        <?php endif; ?>

        <span>Halaman <?= $page ?> dari <?= $total_pages ?></span>

        <?php if ($page < $total_pages): ?>
            <a class="button" href="?c=jarak&a=index&page=<?= $page + 1 ?>">Berikutnya ⟶</a>
        <?php endif; ?>
    </div>
</div>