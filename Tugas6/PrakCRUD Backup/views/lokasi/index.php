<!-- 🌟 CSS sama seperti minimarket/index.php -->
<style>
    body {
...
<div class="">
    ...
    <div class="toolbar">
        <form method="GET">
            <input type="hidden" name="c" value="lokasi">
            <input type="hidden" name="a" value="index">

            <!-- 🔍 Pencarian -->
            <input type="text" name="q" placeholder="Cari alamat / kota / provinsi..." 
            ...
            <div class="sort-group">
                <select name="sort">
                    ...
                    <option value="alamat" <?= (isset($_GET['sort']) && $_GET['sort']=='alamat') ? 'selected' : '' ?>>Alamat</option>
                    <option value="kota" <?= (isset($_GET['sort']) && $_GET['sort']=='kota') ? 'selected' : '' ?>>Kota</option>
                    <option value="provinsi" <?= (isset($_GET['sort']) && $_GET['sort']=='provinsi') ? 'selected' : '' ?>>Provinsi</option>
                </select>
                ...
                <div class="sort-buttons">
                    ...
                    ?>
                    <a href="?c=lokasi&a=index&sort=<?= $currentSort ?>&order=asc&q=<?= $q ?>" 
                       class="button <?= $currentOrder==='asc'?'active':'' ?>">🔼</a>
                    <a href="?c=lokasi&a=index&sort=<?= $currentSort ?>&order=desc&q=<?= $q ?>" 
                       class="button <?= $currentOrder==='desc'?'active':'' ?>">🔽</a>
                </div>
            </div>

            <button type="submit">🔍 Cari</button>
            <a href="?c=lokasi&a=index" class="button">🔄 Reset</a>
        </form>
    </div>
    ...
    if (isset($_GET['q']) && $_GET['q'] !== '') {
        ...
        $data = array_filter($data, function($item) use ($q) {
            return (
                ...
                strpos(strtolower($item['provinsi']), $q) !== false
            );
        });
    }
    ...
    <div class="pagination">
        <?php if ($page > 1): ?>
            <a class="button" href="?c=lokasi&a=index&page=<?= $page - 1 ?>">⟵ Sebelumnya</a>
        <?php endif; ?>

        <span>Halaman <?= $page ?> dari <?= $total_pages ?></span>

        <?php if ($page < $total_pages): ?>
            <a class="button" href="?c=lokasi&a=index&page=<?= $page + 1 ?>">Berikutnya ⟶</a>
        <?php endif; ?>
    </div>
</div>